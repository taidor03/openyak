...
# xhtml2pdf is imported lazily inside html_to_pdf() because its transitive
# dependency oscrypto crashes on macOS 26+ (platform.mac_ver() returns
# empty version-info tuple on Darwin ≥ 26, and oscrypto tries int('')).
# Deferring the import keeps backend startup healthy; only PDF export is
# affected on incompatible platforms.
_XHTML2PDF_IMPORT_ERROR: Exception | None = None
_DEFAULT_FONT_REF: dict | None = None  # cached xhtml2pdf.default.DEFAULT_FONT
...
def _try_register_font(family: str, css_name: str, font_path: Path, is_ttc: bool) -> bool:
    ...
    """
    if _DEFAULT_FONT_REF is None:
        return False  # xhtml2pdf not importable
    ...
    try:
        ...
        _DEFAULT_FONT_REF[css_name] = rl_name
...
def html_to_pdf(html_body: str) -> bytes:
    ...
    Raises RuntimeError if xhtml2pdf is not available (e.g. on macOS 26+
    where oscrypto is incompatible).
    """
    global _XHTML2PDF_IMPORT_ERROR, _DEFAULT_FONT_REF

    # Lazy-import xhtml2pdf so oscrypto bugs don't block backend startup.
    if _DEFAULT_FONT_REF is None:
        if _XHTML2PDF_IMPORT_ERROR is not None:
            raise RuntimeError(
                "PDF export is unavailable: xhtml2pdf could not be imported. "
                "This is a known issue on macOS 26+. "
                "Original error: %s" % _XHTML2PDF_IMPORT_ERROR
            )
        try:
            from xhtml2pdf import pisa as _pisa
            from xhtml2pdf.default import DEFAULT_FONT as _df
            _DEFAULT_FONT_REF = _df
        except Exception as exc:
            _XHTML2PDF_IMPORT_ERROR = exc
            log.error("Cannot import xhtml2pdf — PDF export disabled: %s", exc)
            raise RuntimeError(
                "PDF export is unavailable: xhtml2pdf could not be imported. "
    ...
    buf = io.BytesIO()
    pisa_status = _pisa.CreatePDF(full_html, dest=buf, encoding="utf-8")
