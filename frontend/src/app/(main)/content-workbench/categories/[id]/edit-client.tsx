...
function EditCategoryForm({ id }: { id: string }) {
  ...
  const mut = useMutation({ mutationFn: (d: { name: string }) => updateCategory(id, d), onSuccess: () => { qc.invalidateQueries({ queryKey: ["xflow", "categories"] }); router.push("/content-workbench/categories"); } });
  return (
