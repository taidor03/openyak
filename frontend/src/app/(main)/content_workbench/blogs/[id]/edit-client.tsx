...
function EditBlogForm({ id }: { id: string }) {
  ...
  const mut = useMutation({ mutationFn: (d: { title: string }) => updateBlog(id, d), onSuccess: () => { qc.invalidateQueries({ queryKey: ["xflow", "blogs"] }); router.push("/content_workbench/blogs"); } });
  return (
