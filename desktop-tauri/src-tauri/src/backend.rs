...
impl BackendState {
    ...
    pub async fn start(&self, app: &AppHandle) -> Result<String, String> {
        ...
        let mut child = if is_dev {
            ...
            // macOS 26+ ships a libexpat missing _XML_SetAllocTrackerActivationThreshold
            // which the bundled pyexpat.so needs. Prefer our bundled Homebrew libexpat in
            // the _internal/ directory (sibling of the backend binary).
            #[cfg(target_os = "macos")]
            {
                let dyld_path = backend_path
                    .parent()
                    .unwrap_or(Path::new("."))
                    .join("_internal");
